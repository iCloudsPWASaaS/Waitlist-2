...
@section('content')
<!-- shop section -->
<section class="shop-section" style="margin-top: 150px">
    <div class="container">
                        ...
                        <div class="filter-box">
                            ...
                            <div class="row g-3">
                                <div class="input-box col-12">
                                    <input type="text" class="form-control" name="name" value="{{ old('name', request()->name) }}" autocomplete="off" placeholder="@lang('What are you looking for?')" />
                                </div>
                                ...
                                <div class="input-box col-12">
                                    <select class="js-example-basic-single form-control" name="location">
                                        ...
                                        <option value="{{ $address->id }}" @if(request()->location == $address->id) selected @endif>@lang(optional($address->details)->title)</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="filter-box">
                            ...
                            <div class="check-box searchAmenities">
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="amenity_id[]" @if(isset(request()->amenity_id))
                                    @foreach(request()->amenity_id as $data)
                                    @if($data == $amenity->id) checked @endif
                                    @endforeach
                                    @endif
                                    value="{{ $amenity->id }}" id="amenity{{ $amenity->id }}"/>
                                    <label class="form-check-label" for="amenity{{ $amenity->id }}"><i class="{{ $amenity->icon }}" aria-hidden="true"></i> @lang(optional($amenity->details)->title)
                                    </label>
                                </div>
                                @endforeach
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.data == 'added') {
                    ...
                    $(".item-in-cart-wish").removeClass("d-none");
                    $(".item-in-cart-wish").html(Number($(".item-in-cart-wish").html()) + 1);
                }
                if (data.data == 'remove') {
