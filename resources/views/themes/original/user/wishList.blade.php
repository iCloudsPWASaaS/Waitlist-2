    ...
    <div class="container-fluid">
        <div class="main row">
            <div class="col-12">
                ...
                <div class="search-bar my-search-bar d-none">
                ...
                <div class="table-parent table-responsive wishlistTable">
                    <table class="table table-striped">
                        ...
                        @forelse ($favourite_properties as $key => $wishlist)
                            ...
                            <tr>
                                <td class="company-logo" data-label="@lang('Property')">
                                    <div>
                                        <a href="{{ route('user.propertyDetails',[@slug(optional($wishlist->get_property->details)->property_title), optional($wishlist->get_property)->id]) }}" target="_blank">
                                            <img src="{{ getFile(config('location.propertyThumbnail.path').optional($wishlist->get_property)->thumbnail) }}">
                                        </a>
                                    </div>
                                    <div>
                                        <a href="{{ route('user.propertyDetails',[@slug(optional($wishlist->get_property->details)->property_title), optional($wishlist->get_property)->id]) }}" target="_blank">@lang(\Illuminate\Support\Str::limit(optional($wishlist->get_property->details)->property_title, 30))</a> <br>
                                    </div>
                                </td>
                        ...
                        @empty
                            <td class="text-center" colspan="100%"> <span>There’s nothing on your wishlist. <a href="{{ route('user.propertyMarket', 'investment-properties') }}">Browse</a> properties now.</span></td>
                        @endforelse
                        </tbody>
                    </table>
