    ...
    <div class="container">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="#">{{ @$blogCategory->name }}</a></li>
                <li class="breadcrumb-item active" aria-current="page">@lang(@$singleBlog->details->title)</li>
            </ol>
        </nav>
        ...
        <div class="row g-lg-5">
            <div class="col-lg-8">
                <div class="blog-box">
                    <div class="img-box">
                        <img src="{{ getFile(config('location.blog.path'). @$singleBlog->image) }}" alt="@lang('not found')" class="img-fluid" />
                    </div>
                ...
                <div class="all-comment">
                    ...
                    <div class="comment-box">
                        <div class="img-box">
                            <img class="img-fluid" src="{{getFile(config('location.user.path').@$value->user->image)}}" alt="@lang('not found')" />
                        </div>
                        <div class="text-box">
                            ...
                            <button class="reply-btn replyBtn" data-id="{{ $value->id }}"><i class="fal fa-reply"></i> @lang('Reply')</button>
                                    ...
                                    <div class="secondReplyForm{{ $reply->id }} d-none">
                                        <form action="{{ route('user.sendReply', $reply->id) }}" method="post">
                                            ...
                                            <div class="input-box mb-4">
                                                <textarea class="form-control" name="reply" id="exampleFormControlTextarea1" rows="5" placeholder=""></textarea>
                                            </div>
                    ...
                    <form action="{{ route('user.sendComment', $singleBlog->id) }}" method="post">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="input-box col-12">
                                <textarea class="form-control" name="comment" cols="30" rows="3" placeholder="@lang('Write Message')"></textarea>
                                @error('name')
            ...
            <div class="col-lg-4">
                <div class="side-bar">
                    ...
                    <div class="side-box">
                        ...
                        <div class="blog-box">
                            <div class="img-box">
                                <img class="img-fluid" src="{{ getFile(config('location.blog.path'). @$blog->image) }}" alt="@lang('not found')" />
                            </div>
                            <div class="text-box">
                                <span class="date">{{ dateTime(@$blog->created_at, 'M d, Y') }}</span>
                                <a href="{{route('blogDetails',[slug(@$blog->details->title), $blog->id])}}" class="title">{{ \Illuminate\Support\Str::limit(optional(@$blog->details)->title, 100) }}
                                </a>
                            </div>
                        </div>
                        @endforeach
