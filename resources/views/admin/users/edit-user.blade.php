                ...
                <div class="card card-primary">
                    <div class="card-body">
                        ...
                        <ul class="list-group">
                            ...
                            <!--extra-->
                            <!--<li class="list-group-item d-flex justify-content-between align-items-center">@lang('Interest Balance')
                                <span>@lang(config('basic.currency_symbol')){{ getAmount($user->interest_balance, config('basic.fraction_number'))}} </span>
                            </li>-->
                                    ...
                                    <div class="d-flex no-block align-items-center">
                                        <div class="row">
                                            @foreach($allBadges as $badge)
                                                <div class="badge-box badge-box-two {{ $user->getBadges($badge->id) == true ? '' : 'locked' }}" id="badge-box-two">
                                                    <img src="{{ getFile(config('location.badge.path').$badge->badge_icon) }}" alt=""  data-toggle="tooltip" data-placement="top" title="{{ optional($badge->details)->rank_level }} ({{ optional($badge->details)->rank_name }})"/>
                                                    <div class="lock-icon">
                        ...
                        <div class="btn-list ">
                            ...
                            @if(adminAccessRoute(config('role.manage_user.access.view')))
                                ...
                                <a href="{{ route('admin.user.commissionLog',$user->id) }}"
                                   class="btn btn-outline-primary btn-rounded btn-sm">
                                    <span class="btn-label"><i class="fas fa-newspaper"></i></span> @lang('Commissions')
                                </a>

                                <a href="{{ route('admin.user.plan-purchaseLog',$user->id) }}"
                                   class="btn btn-outline-primary btn-rounded btn-sm">
                                <span class="btn-label"><i
                                        class="fas fa-tree"></i></span> @lang('Investments Log')
                                </a>
                            @endif
                            @if(adminAccessRoute(config('role.manage_user.access.edit')))
            ...
            <div class="modal-content">
                <form method="post" action="{{ route('admin.user-balance-update',$user->id) }}"
                    ...
                    <div class="modal-body">
                        <div class="form-group ">
                            <label>{{trans('Select Wallet')}}</label>
                            <select name="walet" class="form-control">
                                <option value="" disabled>@lang('Select Walet')</option>
                                <option value="main_balance" class="text-white bg-dark">{{trans('Main balance')}}</option>
