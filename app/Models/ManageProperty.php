...
class ManageProperty extends Model
{
    ...
    protected $casts = ['amenity_id' => 'array', 'facility_id' => 'array']; //extra


    protected $appends = ['investmentAmount', 'limitamenity', 'allamenity', 'managetime', 'limitfacility'];
    ...
    //extra
    public function getLimitFacilityAttribute()
    {
        $genres = is_array($this->facility_id) ? $this->facility_id : [$this->facility_id];
        return Facility::whereIn('id', $genres)->limit(3)->orderBy('id', 'ASC')->get();
    }

    public function getPropertydealAttribute()
    {
        return PropertyDeal::where('id', $this->type_of_deal)->first();
    }

    public function getPropertytypeAttribute()
    {
        return PropertyType::where('id', $this->type_of_property)->first();
    }
