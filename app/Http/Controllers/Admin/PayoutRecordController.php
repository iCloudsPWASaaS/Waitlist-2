...
class PayoutRecordController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        $records = PayoutLog::with('user', 'method')
    ...
    public function payoutConfirm(Request $request, $id)
    {
        ...
        //extra
        $user = $payout->user;
        $user->interest_balance -= getAmount($payout->net_amount);
        $user->save();
        ...
        $transaction->trx_type = '-';
        $transaction->remarks = getAmount($payout->amount) . ' ' . $basic->currency . ' withdraw amount has been approved';
