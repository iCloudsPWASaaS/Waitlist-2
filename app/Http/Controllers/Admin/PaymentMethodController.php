...
{
    use Upload;
    public function index(Gateway $gateway) //extra
    {
        //$data['methods'] = Gateway::automatic()->orderBy('sort_by', 'asc')->get();
        //$data['methods'] = Gateway::automatic()->whereIn('id', [1,2])->orderBy('sort_by', 'asc')->get();
        $data['methods'] = Gateway::automatic()->where('status', 1)->orderBy('sort_by', 'asc')->get();
