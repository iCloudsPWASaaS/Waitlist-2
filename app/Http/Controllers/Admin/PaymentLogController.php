...
class PaymentLogController extends Controller
{
    ...
    public function index()
    {
        ...
        $funds = Investment::where('status', '!=', 0)->where('plan_id', null)->orderBy('id', 'DESC')->with('user', 'gateway')->paginate(config('basic.paginate'));
    ...
    public function pending()
    {
        ...
        //$funds = Investment::where('status', 2)->where('gateway_id','>', 999)->orderBy('id', 'DESC')->with('user', 'gateway')->paginate(config('basic.paginate'));
        $funds = Investment::where('status', 2)->orderBy('id', 'DESC')->with('user', 'gateway')->paginate(config('basic.paginate'));
        return view('admin.payment.logs', compact('funds', 'page_title'));
    }
    ...
    public function search(Request $request)
    {
        ...
        $funds = Investment::with('user', 'gateway')
        ...
        ->when(isset($search['property_reference']), function ($query) use ($search) {
            return $query->where('property_reference',  'LIKE', "%{$search['property_reference']}%");
        })
    ...
    public function action(Request $request, $id)
    {
        ...
        $data = Investment::where('id', $request->id)->whereIn('status', [2])->with('user', 'gateway', 'property')->firstOrFail();
        ...
        if ($request->status == '1') {
            $data->status = 1;
            //$data->feedback = $request->feedback; //extra
            $data->update();
