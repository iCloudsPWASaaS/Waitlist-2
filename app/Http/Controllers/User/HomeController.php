...
class HomeController extends Controller
{
     ...
     */
    public function index(Request $request) //extra
    {
        ...
        $data['investment'] = collect(Investment::with('property')->where('user_id', $this->user->id)
            ...
            ->selectRaw('SUM(CASE WHEN invest_status = 1 AND status = 1 AND is_active = 1 THEN amount * card_quantity END) AS runningInvestAmount')
            ->selectRaw('COUNT(id) AS totalInvestment')
            ->selectRaw('SUM(CASE WHEN invest_status = 1 AND status = 1 AND is_active = 1 THEN card_quantity END) AS runningInvestment')
        ...
        //extra
        $data['myPropertiesInvested'] = Investment::with(['property.getReviews', 'propertyShare', 'user'])
            ->where('user_id', $this->user->id)->where('is_active', 1)
            ->where('invest_status', '!=', 4)->where('status', '=', 1)
            ->groupBy('property_id')->latest()->take(7)
            ->selectRaw('*, SUM(amount * card_quantity) AS total_amount')
            ->get();

        $data['myPropertiesPending'] = Investment::with(['property.getReviews', 'propertyShare', 'user'])->where('user_id', $this->user->id)->where('is_active', 1)->where('invest_status', '!=', 4)->where('status', '=', 0)->groupBy('property_id')->latest()->take(7)->get();
        $data['allAddress'] = Address::with('details')->where('status', 1)->get();


        /* $data['monthlyInvestmentExtra'] =
            Investment::where('user_id', $this->user->id)->where('is_active', 1)->where('invest_status', '!=', 4)->where('status', '=', 1)
            ->whereBetween('created_at', [
            ...
            /* $data['monthlyInvestmentExtra'] = Investment::where('user_id', $this->user->id)->groupBy(DB::raw("MONTH(created_at)"))
            ->where('is_active', 1)->where('invest_status', '!=', 4)->where('status', '=', 1)
            ->selectRaw('SUM(amount * card_quantity) AS total_amount')
            ->selectRaw('SUM(profit * card_quantity) AS total_profit')->first(); */

            //dd($data['monthlyInvestmentExtra']->total_amount);

        /* $data['monthlyInvestmentProfitExtra'] = Investment::where('user_id', $this->user->id)->where('is_active', 1)->where('invest_status', '!=', 4)->where('status', '=', 1)
            ->withSum('properties', 'capital_appreciation')
            ->withSum('properties', 'property_yield')
            ->selectRaw('SUM(amount * card_quantity) AS total_amount')
            ->selectRaw('sum(card_quantity) as card_quantity')
            //->groupBy(DB::raw("user_id"))
            //->sum('card_quantity')
            ->get(); */

            $data['monthlyInvestmentProfitExtra'] = DB::table('investments')
            ->join('manage_properties', 'investments.property_id', '=', 'manage_properties.id')
            ->select('investments.*', \DB::raw('SUM(investments.amount * investments.card_quantity) as total_amount'), \DB::raw('SUM(investments.card_quantity) as card_quantity'), \DB::raw('SUM(manage_properties.capital_appreciation) as capital_appreciation'), \DB::raw('SUM(manage_properties.property_yield) as property_yield'), \DB::raw('SUM(investments.profit * investments.card_quantity) as total_profit'))
            ->where('user_id', $this->user->id)->where('is_active', 1)->where('invest_status', '!=', 4)->where('investments.status', '=', 1)
            ->groupBy(DB::raw("user_id"))
            ->first();

        //$data['monthlyInvestmentProfitExtra'] = $data['monthlyInvestmentProfitExtra'] ? $data['monthlyInvestmentProfitExtra'] : 0;
        //dd($data['monthlyInvestmentProfitExtra']?->total_profit);
    ...
    public function fundHistory() //extra
    {
        ...
        /* $first = DB::table("funds")
            ->select(
                DB::raw('funds.id'),
                DB::raw('funds.created_at'),
                DB::raw("funds.amount"),
                DB::raw("funds.status"),
                DB::raw("'Paid' as type"),
            )
            ->where('funds.user_id', $this->user->id)->where('funds.status', '!=', 0); */

        $first = DB::table("investments")
            ->select(
                DB::raw('investments.id'),
                DB::raw('investments.created_at'),
                DB::raw("investments.amount"),
                DB::raw("investments.status"),
                DB::raw("'Paid' as type"),
                DB::raw("investments.property_reference as property_reference")
            )
            ->where('investments.user_id', $this->user->id)->where('investments.status', '!=', 0);

        $funds = DB::table("payouts")
            ->select(
                DB::raw('payouts.id'),
                DB::raw('payouts.created_at'),
                DB::raw("payouts.amount"),
                DB::raw("payouts.status"),
                DB::raw("'Withdrawal' as type"),
                DB::raw("'N/A' as property_reference")
            )
            ->where('payouts.user_id', $this->user->id)
            ->union($first)
            ->paginate(config('basic.paginate'));

        $transactions = $this->user->transaction()->get();
        //dd($transactions);

        //$balance = $this->user->transaction()->where('balance_type', '=','balance')->sum('amount');
        //$interest_balance = $this->user->transaction()->where('balance_type', 'interest_balance')->sum('amount');

        $balance = $this->user->balance;
        //$interest_balance = $this->user->interest_balance + $this->user->joining_bonus + $this->user->total_referral_bonous + $this->user->total_badge_bonous;
        $interest_balance = $this->user->interest_balance;

        /* dd($funds);
        die; */

        return view($this->theme . 'user.transaction.fundHistory', compact('funds', 'balance', 'interest_balance', 'transactions'));
    }
    ...
    public function investProperty(Request $request, $id) //extra
    {
        $rules = [
        ...
        $investments = Investment::with('property.details')->where('user_id', Auth::id())->where('is_active', 0)->where('invest_status', '!=', 4)->where('property_id', $id)->where('deleted_at', '=', Null)->exists();

        if ($investments) {
            return response()->json(['error' => "Property card is already in your cart"], 422);
        } else {

            $user = $this->user;

            //$property = ManageProperty::with('details')->where('id', $id)->where('status', 1)->where('expire_date', '>', now())->first();
            $property = ManageProperty::with('details')->where('id', $id)->where('status', 1)->first();

            if (!$property) {
                //return back()->with('error', 'Invalid Invest Request');
                return response()->json(['error' => 'Invalid Invest Request'], 422);
            }


            //$balance_type = $request->balance_type;
            //$balance_type = 'checkout';

            //$pay_installment = $request->pay_installment;
            $pay_installment = null;

            /* if (!in_array($balance_type, ['balance', 'interest_balance', 'checkout'])) {
                //return back()->with('error', 'Invalid Wallet Type');
                return response()->json(['error' => 'Invalid Wallet Type'], 422);
            } */
            ...
            if (($property->fixed_amount != null && $amount != $property->fixed_amount) && ($pay_installment == null && $property->available_funding != $amount)) {
                //return back()->with('error', "Please invest " . $property->fixed_amount);
                return response()->json(['error' => 'Please invest ' . $property->fixed_amount], 422);
            }
            ...
            $user->total_invest += $request->amount; //extra
            $user->save(); */

            $trx = strRandom();
            $remarks = 'Invested On ' . optional($property->details)->property_title;

            //BasicService::makeTransaction($user, $amount, 0, '-', $balance_type, $trx, $remarks, $property);  //extra

            InvestmentService::profitLossCalculate($request, $property);
            InvestmentService::forAllInvest($user, $property, $amount, $pay_installment, $trx);
            ...
            return response()->json(['success' => 'Property card has been added to cart.']);
        }
    }

    public function investPropertyCart() //extra
    {
        //\DB::enableQueryLog();
        $investments = Investment::with('property.details')->where('user_id', Auth::id())->where('is_active', 0)->where('invest_status', '!=', 4)->get();
        //dd(\DB::getQueryLog());
        //die;

        $data['gateways'] = Gateway::where('status', 1)->orderBy('sort_by', 'DESC')->get();

        return view($this->theme . 'user.cart', $data, compact('investments'));
    }

    public function investPropertyCartSubmit(Request $request) //extra not-in-use
    {
        /* dd($request);
        die; */
        ...
        BasicService::makeTransaction($user, $amount, 0, '-', $balance_type, $trx, $remarks);  //extra



        foreach ($request->investments as $investment) {
            $myInvest = Investment::findOrFail($investment);
            $myInvest->is_active = 1;

            //extra
            $myInvest->gateway_id = $gate->id;
            $myInvest->gateway_currency = strtoupper($gate->currency);
            $myInvest->charge = $charge;
            $myInvest->rate = $gate->convention_rate;
            $myInvest->final_amount = getAmount($final_amo);
            $myInvest->btc_amount = 0;
            $myInvest->btc_wallet = "";
            $myInvest->transaction = $trx;
            $myInvest->try = 0;
            $myInvest->gateway_status = 0;
        ...
        return back()->with('success', '`' . optional($property->details)->property_title . '`' . ' has been invested successfully.');
        return back()->with('success', 'You have been invested successfully.');*/

        return response()->json(['success' => 'You have been invested successfully.']);
    }

    public function investPropertyCartRemove(Request $request) //extra
    {
        /* dd($request->invest_id);
        die; */

        $selectedInvestId = $request->invest_id;
        Investment::findOrFail($selectedInvestId)->delete();
    ...
    public function reserveProperty(Request $request, $id) //extra
    {
        $investments = Investment::with('property.details')->where('user_id', Auth::id())->where('is_active', 0)->where('property_id', $id)->where('deleted_at', '=', Null)->where('invest_status', 3)->exists();

        if ($investments) {
            return response()->json(['error' => "Card alreay exist on your reserve list"], 422);
        } else {
            ...
            $trx = strRandom();
            $remarks = 'Reserved On ' . optional($property->details)->property_title;
            InvestmentService::profitLossCalculate($request, $property);
            InvestmentService::forAllInvest($user, $property, $amount, $pay_installment, $trx, true);
            return response()->json(['success' => 'Property has been reserved.']);
        }
    }
    ...
    public function propertyShareStore(Request $request, $id)
    {
        ...
        $propertyShare->save();
        return redirect()->route('user.shareMarket', 'property-share-market')->with('success', 'Share successfully saved!'); //extra
    }
    ...
    public function payoutPreview()
    {
        ...
        $layout = 'layouts.user';
        $remaining = getAmount(auth()->user()->interest_balance - $withdraw->net_amount); //extra
    ...
    public function propertyMarket(Request $request, $type = null) //extra
    {
        /* dd($type);
        die; */
        ...
        /*dd($data['properties'][0]->image[0]->id);
            die;*/

        if ($type == 'issued-properties') {
            //\DB::enableQueryLog();
            ...
            /* $data['myProperties'] = Investment::with(['property.getReviews', 'propertyShare', 'user'])->whereHas('property', function ($query) {
                return $query->whereRaw('card_issued > card_remain');
            })->where('user_id', $this->user->id)->groupBy('property_id')->latest()->paginate(config('basic.paginate')); */

            $data['myProperties'] = Investment::with(['property.getReviews', 'propertyShare', 'user'])
            ->where('user_id', $this->user->id)->groupBy('property_id')
            ->selectRaw('*, SUM(amount * card_quantity) AS total_amount')
            ->latest()->paginate(config('basic.paginate'));

            /* dd(\DB::getQueryLog());
            dd($data['myProperties']);
            die; */
        } else if ($type == 'sold-properties') {
            /* $data['myProperties'] = Investment::with(['property.getReviews', 'propertyShare', 'user'])->whereHas('property', function ($query) {
                return $query->whereRaw('card_issued = card_remain');
            })->where('user_id', $this->user->id)->groupBy('property_id')->latest()->paginate(config('basic.paginate')); */
        } else {
        ...
        return view($this->theme . 'user.property.index', $data, compact('type', 'min', 'max', 'minRange', 'maxRange'));
    }



    public function shareMarket($type = null) //extra
    {
        $data['properties'] = ManageProperty::with(['details', 'getInvestment', 'getAddress.details', 'getReviews'])
            ->withCount('getReviews')
            ->withCount('getFavourite')
        ...
        $data['sharedProperties'] = PropertyShare::with(['getInvestment'])->where('status', 1)->latest()->paginate(config('basic.paginate'));
        return view($this->theme . 'user.property.share', $data, compact('type'));
    }
    ...
    public function BuyShare(Request $request, $id) //extra
    {
        $this->validate($request, [
            //'balance_type' => 'required|string|max:50',
            'amount' => 'required|numeric',
        ...
        return redirect()->route('user.propertyMarket', 'issued-properties')->with('success', 'A share of' . '`' . optional(optional($propertyShare->property)->details)->property_title . '`' . 'property has been successfully purchased.');
    }
    ...
    public function premiumUser(Request $request)
    {
        ...
        return response([
            'status' => 'Your plan has been upgrded as Premium User'
        ]);
    }

    public function searchPropertyDashboard(Request $request)
    {
        $search = $request->all();
        $fromDate = Carbon::parse($request->from_date);
        $toDate = Carbon::parse($request->to_date)->addDay();

        $investments = $this->user->invests()->with('property.details', 'propertyShare', 'property.getAddress.details')->where('is_active', 1)->where('invest_status', '!=', 4)->where('status', '=', 1)
            ->when(isset($search['property']), function ($query) use ($search) {
                return $query->whereHas('property.details', function ($q) use ($search) {
                    $q->where('property_title', 'LIKE', '%' . $search['property'] . '%');
                });
            ...
            ->when(isset($search['location']), function ($query) use ($search) {
                return $query->whereHas('property.getAddress', function ($query2) use ($search) {
                    $query2->where('id', 'LIKE', "%{$search['location']}%");
                });
            })
            ->when(isset($search['invest_status']), function ($query) use ($search) {
    ...
    {
        $page_title = "Balance Transfer";
        return view($this->theme . 'user.market', compact('page_title'));
    }
