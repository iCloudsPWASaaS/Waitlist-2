...
class RegisterController extends Controller
{
    ...
    */
    ...
    protected function validator(array $data)
    {
        ...
        $rules['terms_conditions'] = ['required']; //extra
        $rules['password_confirmation'] = ['required']; //extra
        return Validator::make($data, $rules, [
            'firstname.required' => 'First Name Field is required',
            'lastname.required' => 'Last Name Field is required',
            'g-recaptcha-response.required' => 'The reCAPTCHA field is required.',
            'terms_conditions.required' => 'Please check terms & conditions.',
        ]);
    }
    ...
    public function join(Request $request) //extra
    {
        //dd("working");
        //$this->validator($request->all())->validate();
        ...
        $currentDate = dateTime(Carbon::now());
         $this->sendMailSms($user, $type = 'REGISTER_CONFIRM_MAIL_TO_USER_2', [
            'name'          => $user->fullname,
            'date'          => $currentDate,
            'link'          => route('join.sponsor',[$user->username]),
