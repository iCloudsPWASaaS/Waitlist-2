...
function investmentCartCount() //extra
{
    $investments = \App\Models\Investment::with('property.details')->where('user_id', auth()->id())->where('is_active', 0)->where('invest_status', '!=', 4)->get();
    return count($investments);
}

function propertyWishCount() //extra
{
    $wish = \App\Models\Favourite::where('client_id', auth()->id())->get();
    return count($wish);
}
