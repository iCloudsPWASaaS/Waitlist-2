...
class PackageService
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            // if (is_null(Package::where(['status' => 1])->first())) {
            //     Package::first()->update(['status' => 1]);
            // }

            // // update if trail changed
            // if (is_null(Package::where(['is_trail' => 1, 'status' => 1])->first())) {
            //     Package::where(['status' => 1])->first()->update(['is_trail' => 1]);
            // }
    ...
    public function destroy($id)
    {
        DB::beginTransaction();
        try {
            if (Package::where('status', 1)->count() > 1) {
